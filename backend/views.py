...
def contract_list(request):
    ...
    # return render(request, 'backend/contract_list.html', {'contracts': contracts, 'filterContract': filterContract})
@login_required
...
def invoice_list(request):
    invoices = Invoice.objects.all()  # Ensure all invoices are retrieved
...
def invoice_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('invoice_list')  # Ensure redirection to invoice_list
    else:
        form = InvoiceForm()
